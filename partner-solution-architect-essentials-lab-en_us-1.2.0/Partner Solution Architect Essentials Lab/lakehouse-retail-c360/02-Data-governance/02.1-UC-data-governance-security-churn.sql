...
-- We'll grant an extra MODIFY to our Data Engineer (dynamically using current catalog/schema)
DECLARE OR REPLACE stmt = CONCAT('GRANT SELECT, MODIFY ON SCHEMA ', CURRENT_CATALOG(), '.', CURRENT_SCHEMA(), ' TO `dataengineers`');
...
DROP TABLE IF EXISTS churn_users_protected;
