...
# MAGIC %md
# MAGIC
# MAGIC This function utilizes the churn prediction model we defined in [Section 4]($../04-Data-Science-ML/04.1-churn-prediction) of this demo.
...
RETURNS STRING
LANGUAGE SQL
COMMENT 'This function determines whether a customer is at risk of churning.'
RETURN
(
    SELECT CASE 
        WHEN ai_query(
            ...
            request => named_struct(
                ...
                'days_last_event', days_last_event
            ),
            returnType => 'STRING'
            ) = '0' THEN 'NOT AT RISK'
        ELSE 'AT RISK'
    END
    FROM churn_user_features
    WHERE user_id = id
    LIMIT 1
)
""")
