...
spark.createDataFrame(expectations, headers.split(",")) \
  ...
  .write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(f"{catalog}.{db}.dlt_expectations")

# COMMAND ----------

spark.sql(f'alter table {catalog}.{db}.dlt_expectations set tblproperties (delta.autoOptimize.optimizeWrite = true, delta.autoOptimize.autoCompact = true)')
