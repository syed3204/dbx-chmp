...
COMMENT "Customer at risk of churn"
  AS SELECT predict_churn(struct(
    user_id,
    COALESCE(CAST(age_group AS LONG), 0) as age_group,
    COALESCE(canal, 'UNKNOWN') as canal,
    COALESCE(country, 'UNKNOWN') as country,
    COALESCE(CAST(gender AS LONG), 0) as gender,
    COALESCE(CAST(order_count AS LONG), 0) as order_count,
    COALESCE(CAST(total_amount AS LONG), 0) as total_amount,
    COALESCE(CAST(total_item AS LONG), 0) as total_item,
    COALESCE(last_transaction, now()) as last_transaction,
    COALESCE(platform, 'other') as platform,
    COALESCE(CAST(event_count AS LONG), 0) as event_count,
    COALESCE(CAST(session_count AS LONG), 0) as session_count,
    COALESCE(CAST(days_since_creation AS LONG), 0) as days_since_creation,
    COALESCE(CAST(days_since_last_activity AS LONG), 0) as days_since_last_activity,
    COALESCE(CAST(days_last_event AS LONG), 0) as days_last_event
  )) as churn_prediction, * FROM live.churn_features
