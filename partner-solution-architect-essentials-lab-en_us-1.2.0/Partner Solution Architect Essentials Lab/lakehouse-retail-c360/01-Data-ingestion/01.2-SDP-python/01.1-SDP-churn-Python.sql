...
-- MAGIC #If you have issues, make sure this matches your model dependency version. For prod usage, use env_manager='conda'
...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&notebook=01.3-DLT-churn-python&demo_name=lakehouse-retail-c360&event=VIEW">
...
-- MAGIC
-- MAGIC *Note that we're including the ML model our [Data Scientist built]($../04-Data-Science-ML/04.1-churn-prediction) to predict the churn. We'll cover that in the next section.*
...
-- MAGIC
-- MAGIC <div><img src="https://github.com/QuentinAmbard/databricks-demo/raw/main/retail/resources/images/lakehouse-retail/lakehouse-retail-churn-de-small-1.png" width="700px" style="float: right"/></div>
-- MAGIC
...
-- MAGIC
-- MAGIC Note that while our streaming data is added to our cloud storage, we could easily ingest from kafka directly.
-- MAGIC
...
-- MAGIC
-- MAGIC <div><img src="https://github.com/QuentinAmbard/databricks-demo/raw/main/retail/resources/images/lakehouse-retail/lakehouse-retail-churn-de-small-2.png" width="700px" style="float: right"/></div>
-- MAGIC
...
-- MAGIC
-- MAGIC <div><img src="https://github.com/QuentinAmbard/databricks-demo/raw/main/retail/resources/images/lakehouse-retail/lakehouse-retail-churn-de-small-3.png" width="700px" style="float: right"/></div>
-- MAGIC
...
-- MAGIC * ...
-- MAGIC
-- MAGIC Our Data scientist team has built a churn prediction model and saved it into Databricks Model registry.
-- MAGIC
...
-- MAGIC %md
-- MAGIC Open the [transformations/03-gold.py]($./transformations/03-gold.py) notebook to review the Python code creating our features and predictions (ML model is loaded in this file).
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="sdp-churn" href="#joblist/pipelines/a6ba1d12-74d7-4e2d-b9b7-ca53b655f39d" target="_blank">Spark Declarative Pipeline</a> is now ready using Python. We have an end-to-end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
-- MAGIC For more details on model training, open the [model training notebook]($../04-Data-Science-ML/04.1-churn-prediction)
-- MAGIC
...
-- MAGIC SDP tracks all your data quality metrics. You can leverage the expectations directly as SQL table with Databricks SQL to track your expectation metrics and send alerts as required. This lets you build the following dashboards:
-- MAGIC
