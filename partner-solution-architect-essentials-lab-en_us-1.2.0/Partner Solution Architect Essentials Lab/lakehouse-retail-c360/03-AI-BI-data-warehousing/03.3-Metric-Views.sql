...
-- MAGIC %md
-- MAGIC # Create a Metric View with Genie Code
-- MAGIC
...
-- MAGIC version: 0.1
-- MAGIC
-- MAGIC source: dbacademy.${schema}.churn_users
-- MAGIC
-- MAGIC dimensions:
...
-- MAGIC     source: dbacademy.${schema}.churn_orders
-- MAGIC     using: ["user_id"]
-- MAGIC ~~~
...
-- MAGIC We created a metric view from the catalog explorer using Genie Code in the last steps. Now, we will create a metric view using SQL. The YAML can be provided in the SQL statement itself.
...
-- MAGIC %md
-- MAGIC ## Use Genie Code to create a visualization
